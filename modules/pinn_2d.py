...
class PINN_plate(nn.Module):
    def __init__(self, id):
        super(PINN_plate, self).__init__()
    ...
    def forward(self, x, y):
        ...
        a_layer4       = act_func(self.hidden_layer4(a_layer3))
        # a_layer5       = act_func(self.hidden_layer5(a_layer4))
        # a_layer6       = act_func(self.hidden_layer6(a_layer5))
        # a_layer7       = act_func(self.hidden_layer7(a_layer6))
        out            = self.output_layer(a_layer4)
...
class BCs():
    def __init__(self, size, x_lb, x_rb, y_lb, y_rb, u, v, deriv_x, deriv_y, E=None):
        self.size = size
        self.x_lb = x_lb
        self.x_rb = x_rb
        self.y_lb = y_lb
...
class PDEs():
    def __init__(self, size, w1, w2, fx, fy, x_lb, x_rb, y_lb, y_rb, E=None):
        self.x_lb = x_lb
        self.x_rb = x_rb
        self.y_lb = y_lb
        self.y_rb = y_rb
...
class PDEs_plate():
    def __init__(self, size, q, D, x_lb, x_rb, y_lb, y_rb):
        self.x_lb = x_lb
        self.x_rb = x_rb
        self.y_lb = y_lb
        self.y_rb = y_rb
...
class Where_2D():
    ...
    def __call__(self, x, y):
        ...
        # out_x = torch.where(res_x > 0, zeros, ones)
        # out_y = torch.where(res_y > 0, zeros, ones)
        return out_x * out_y

class CPINN_2D_surrogate(nn.Module):
    def __init__(self, domain_no, lb_x, rb_x, lb_y, rb_y, figure_path):
        super(CPINN_2D_surrogate, self).__init__()
    ...
    def forward(self, x, y, e):
        ...
        for i, model in enumerate(models_num):
            out += model(x, y, e) * self.wheres[i](x, y)
    ...
    def get_boundary_error_2d(self, size):
        ...
        for bd in bds:
            ...
            if x_lb == x_rb:
                ...
                out += ( calc_deriv(y, self(x - dw, y), 1) - calc_deriv(y, self(x + dw, y), 1) ) ** 2
                # out += ( calc_deriv(y, self(x - dw, y), 2) - calc_deriv(y, self(x + dw, y), 2) ) ** 2
                # out += ( calc_deriv( x, calc_deriv(y, self(x - dw, y), 1), 1 ) - calc_deriv( x, calc_deriv(y, self(x + dw, y), 1), 1 ) ) ** 2
